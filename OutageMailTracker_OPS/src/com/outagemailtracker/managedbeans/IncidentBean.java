...
public class IncidentBean {
	...
	public IncidentBean() {
		...
		this.to="ravinder.varagani@bp.com";
		...
		/*	setCommonStreams();
			...
			this.vendorMailList=new ArrayList<String>();
				...
				this.vendorMailList.addAll(Factory.createVendorDAO().fetchAllMailAddress());
...
//			this.mailSelectItem=null;
	...
	public void assignIssueType(ValueChangeEvent vce) {
		...
		this.issueType=(String)vce.getNewValue();
...
//				System.out.println(Arrays.toString(mailSet.toArray()));
	...
	public String validate() {
		...
		if(this.announcement==null || this.announcement.length()==0 || this.announcement=="") {
		//this.proceed=true;
			this.message="Announcement is Mandatory";
		}
		...
		if((this.region == null || this.region.length==0) && (!this.issueType.equalsIgnoreCase("announcement"))) {
		...
		/*if((this.businessImpact ==null || this.businessImpact =="") && (!this.issueType.equalsIgnoreCase("announcement"))) {
		...
		if((this.escalationContact==null || this.escalationContact=="" || this.escalationContact.length()==0) && (!this.issueType.equalsIgnoreCase("announcement"))) {
			this.message="Escalation Contact is Mandatory";
		}		
		if((this.incident==null || this.incident=="" || this.incident.length()==0) && (!this.issueType.equalsIgnoreCase("announcement"))) {
		...
		if((this.impact_window==null || this.impact_window=="" || this.impact_window.length()==0) && (!this.issueType.equalsIgnoreCase("announcement")))
				{
			this.message="Impact_window Details is Mandatory";
		}
		
		/*if(this.announcement==null || this.announcement=="" || this.announcement.length()==0) {
			this.message="Announcement Details is Mandatory";
		}*/
		
		
		if(this.issueType.equalsIgnoreCase("maintenance") && (!this.issueType.equalsIgnoreCase("announcement")) && this.maintenanceFrom==null) {
			this.message="Maintenance From Time is Mandatory";
		}
		if(this.issueType.equalsIgnoreCase("maintenance") && (!this.issueType.equalsIgnoreCase("announcement")) && this.maintenanceTo==null) {
			this.message="Maintenance To Time is Mandatory";
		}
		
		if((!this.issueType.equalsIgnoreCase("maintenance")) && (!this.issueType.equalsIgnoreCase("announcement")) && (this.rca==null || this.rca.length()==0)) {
...
//		}
		 ...
		 * ADD
		 */
		inc.setImpact_window(this.impact_window);
		inc.setAnnouncement(this.announcement);
		
		System.out.println(this.announcement+"this.announcement");
...
//			this.message="Incident Notification Sent Successfully. INC ID : "+incId;
			if((!this.issueType.equalsIgnoreCase("maintenance")) && (!this.issueType.equalsIgnoreCase("announcement"))) {
			...
			else if((!this.issueType.equalsIgnoreCase("maintenance")) || (this.issueType.equalsIgnoreCase("announcement")))
			{
				if(Factory.createMailService().announcementMailSend(inc).equalsIgnoreCase("success")){
					int incId=Factory.createIncidentService().insertIncident(inc);
					/////////////
					System.out.println(inc.getAnnouncement());
					this.message="Anouncement Notification Sent Successfully. INC ID : "+incId;
				}else {
					this.message="Mail Send Failed";
				}
